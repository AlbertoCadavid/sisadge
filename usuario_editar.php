...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  if ($_POST['cambio_clave'] == "false") {

    $updateSQL = sprintf(
      "UPDATE usuario SET usuario=%s, nombre_usuario=%s, tipo_usuario=%s, codigo_usuario=%s, responsable=%s, fecha_registro=%s, responsable_edit=%s, fecha_edit=%s, email_usuario=%s WHERE id_usuario=%s",
      GetSQLValueString($_POST['usuario'], "text"),
      //no se envia la clave del usuario para que no se encripte de nuevo
    ...
    $updateSQL = sprintf(
      "UPDATE usuario SET usuario=%s, clave_usuario=%s, nombre_usuario=%s, tipo_usuario=%s, codigo_usuario=%s, responsable=%s, fecha_registro=%s, responsable_edit=%s, fecha_edit=%s, email_usuario=%s WHERE id_usuario=%s",
      GetSQLValueString($_POST['usuario'], "text"),
  ...
  $updateSQL2 = sprintf(
    "UPDATE accesos SET acceso=%s, superacceso=%s,no_edita=%s WHERE usuario_id =%s",
    ...
    <table class="table table-bordered table-sm">
      ...
      <tr>
        <td id="dato2"><?php echo $row_usuario['fecha_registro']; ?>
          <input name="fecha_registro" type="hidden" value="<?php echo $row_usuario['fecha_registro']; ?>">
        </td>
        <td colspan="2" id="detalle1"><?php echo $row_usuario['responsable']; ?>
          <input name="responsable" type="hidden" value="<?php echo $row_usuario['responsable']; ?>">
        </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td id="dato2"><input type="text" class="clave_usuario" name="clave_usuario" value="<?php echo $row_usuario['clave_usuario']; ?>" size="20"></td>
        <td id="dato2"><input type="hidden" class="cambio_clave" name="cambio_clave" value=""></td>
      ...
      <tr>
        ...
        <td colspan="2" id="dato2"><label for="email_usuario"></label>
          <input type="text" name="email_usuario" id="email_usuario" value="<?php echo $row_usuario['email_usuario']; ?>" size="50">
        </td>
      </tr>
      ...
      <tr>
        <td id="dato2">
          <select name="tipo_usuario" onChange="DefinicionUsuario(form1.tipo_usuario.value,<?php echo $_SESSION['MM_IdUsername']; ?>);">
            ...
            do {
            ?>
              <option value="<?php echo $row_ver_tipo_user['id_tipo'] ?>" <?php if (!(strcmp($row_ver_tipo_user['id_tipo'], $row_usuario['tipo_usuario']))) {
                                                                            echo "selected=\"selected\"";
                                                                          } ?>><?php echo $row_ver_tipo_user['nombre_tipo'] ?></option>
            <?php
        ...
        <td id="dato2">
          ...
          <select name="no_edita">
            <option value="0" <?php if (!(strcmp("0", $row_acceso['no_edita']))) {
                                echo "selected=\"selected\"";
                              } ?>>NO</option>
            <option value="1" <?php if (!(strcmp("1", $row_acceso['no_edita']))) {
                                echo "selected=\"selected\"";
                              } ?>>SI</option>
        ...
        <td id="dato2">
          <select name="acceso">
            <option value="0" <?php if (!(strcmp("0", $row_acceso['acceso']))) {
                                echo "selected=\"selected\"";
                              } ?>>SIN ACCESO</option>
            <option value="1" <?php if (!(strcmp("1", $row_acceso['acceso']))) {
                                echo "selected=\"selected\"";
                              } ?>>CON ACCESO</option>
        ...
        <td id="dato2">
          <select name="superacceso">
            <option value="0" <?php if (!(strcmp("0", $row_acceso['superacceso']))) {
                                echo "selected=\"selected\"";
                              } ?>>SIN SUPERACCESO</option>
            <option value="1" <?php if (!(strcmp("1", $row_acceso['superacceso']))) {
                                echo "selected=\"selected\"";
                              } ?>>CON SUPERACCESO</option>
      ...
      <tr>
        ...
        <td colspan="2" id="dato2"><?php echo $row_usuario['fecha_edit']; ?> *
          <input name="fecha_edit" type="hidden" value="<?php echo date("Y/m/d"); ?>">
        </td>
      </tr>
      <tr>
        <td colspan="4" id="dato2"><input id="actualizar" type="submit" value="Actualizar  Usuario"></td>
      </tr>
    </table>
...
<script>
  let clave = document.querySelector('.clave_usuario');
  let actualizar = document.querySelector('#actualizar');
  let nuevaClave = document.querySelector('.cambio_clave');
  let password;
  ...
  actualizar.addEventListener('click', function() {
    let newPassword = clave.value;
    if (password == newPassword) {
      nuevaClave.value = false;
    } else {
      nuevaClave.value = true;
    }
