      ...
      if (data) {
        ...
        for (i = 0; i < data.length; i++) {
          ...
          html +=
            ...
            data[i].id +
            "," +
            "'insumos_interno_entrada_salida_edit.php'," +
            data[i].remision_id +
...
function consultasItemsVistaProdTerminado(remision_id) {
      ...
      if (data) {
        ...
        for (i = 0; i < data.length; i++) {
          ...
          html +=
            ...
            "<td>" + data[i].numInicio + "</td>" +
            "<td>" + data[i].numFinal + "</td>" +
            "<td>" + data[i].caja + "</td>" +
            "<td>" + data[i].oc + "</td>" +
            "</tr>";
        }
...
function consultasItemsProdTerminado(remision_id) {
      ...
      if (data) {
        ...
        for (i = 0; i < data.length; i++) {
          ...
          html +=
            ...
            "<td>" + data[i].caja + "</td>" +
            "<td>" + data[i].oc + "</td>" +
            '<td><button onClick="eliminarItemProdTerm(' +
            "'id_items'" +
            "," +
            data[i].id +
            "," +
            "'insumos_devolucion_producto_terminado_edit.php'," +
            data[i].remision_id +
            ')" id="btnDelItems"  name="btnDelItems" type="button" class="botonDel" autofocus="" >DELETE</button></td>' +
            "</tr>";
        }
...
function verConsulta(id, campo, pagina) {
  $.ajax({
    ...
    url:
      "AjaxControllers/Actions/consultas.php?" +
      id +
      "=" +
      campo +
      "&pagina=" +
        ...
        for (i = 0; i < data.length; i++) {
          if (data[i].insumoveinte != "") {
            ...
            $("#verAlert").text(
              "Este insumo de ID: " +
                data[i].insumoveinte +
                " tiene menos de 20 cantidades en stock! "
            );
            $("#verAlert").fadeIn();
            setTimeout(function () {
              $("#verAlert").fadeOut();
...
function consultaGeneralTodosAjax(
  tabla,
  distinct,
  columna1,
  columna2 = "",
    ...
    .done(function (data, textStatus, jqXHR) {
      if (data) {
        ...
        for (i = 0; i < data.length; i++) {
          html +=
            "<tr>" +
            '<td nowrap="nowrap"><input onChange="Updates(' +
            data[i].id +
            ',this)" type="text" required="required" placeholder="Nombre Responsable" id="nombre_responsable" name="nombre_responsable" value="' +
            data[i].nombre_responsable +
            '" class="campostext" ></td>' +
            '<td nowrap="nowrap"><input onChange="Updates(' +
            data[i].id +
            ',this)" type="text" required="required" placeholder="Direccion" id="direccion" name="direccion" value="' +
            data[i].direccion +
            '" class="selectsMMedio"></td>' +
            '<td nowrap="nowrap"><input onChange="Updates(' +
            data[i].id +
            ',this)" type="text" required="required" placeholder="Indicativo" id="indicativo" name="indicativo" value="' +
            data[i].indicativo +
            '" class="campostextMini"></td>' +
            '<td nowrap="nowrap"><input onChange="Updates(' +
            data[i].id +
            ',this)" type="text" required="required" placeholder="Telefono" id="telefono" name="telefono" value="' +
            data[i].telefono +
            '" class="campostextMini"></td>' +
            '<td nowrap="nowrap"><input onChange="Updates(' +
            data[i].id +
            ',this)" type="text" required="required" placeholder="Extension" id="extension" name="extension" value="' +
            data[i].extension +
            '" class="campostextMini"></td>' +
            '<td nowrap="nowrap"><input onChange="Updates(' +
            data[i].id +
            ',this)" type="text" required="required" placeholder="Ciudad" id="ciudad" name="ciudad" value="' +
            data[i].ciudad +
            '" class="campostextMini"></td>' +
            '<td nowrap="nowrap" ><button onClick="eliminacionyConsulta(' +
            "'id_bodega'" +
            "," +
            data[i].id +
            "," +
      ...
      if (data) {
        ...
        for (i = 0; i < data.length; i++) {
          html +=
            ...
            data[i].id +
            "," +
            "'verificacion_cireles.php'," +
            data[i].id_verif +
            ')" id="btnDelItems"  name="btnDelItems" type="button" class="botonDel" autofocus=""  >DELETE</button></td>' +
            '<td><a href="verificacion_cireles_imprime.php?id_imprime=' +
            data[i].id +
            ' " target="_blank"><img src="images/impresor.gif" alt="IMPRIME" title="IMPRIME" border="0" style="cursor:hand;width:20px;height:20px;" /></a></td>' +
            "</tr>";
        }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      object.forEach((element) => {
        clientes[element.id_p] = element.proveedor_p;
      });
        ...
        inputValidator: (value) => {
          return new Promise((resolve) => {
            if (value) {
              ...
              resolve("Debes seleccionar un Proveedores");
            }
          });
        },
      }).then((result) => {
        if (result.isConfirmed) {
          const clienteSeleccionado = result.value;
          var cliente = result.value;
          
          object.forEach((value) => {
            if (value.id_p == clienteSeleccionado) {
              $("#documento").val(value.nit_p);
                $("#pais").val(value.ciudad_p),
                $("#contacto").val(value.contacto_p),
                $("#telefono").val(value.telefono_p),
                $("#celular").val(value.celular_c_p),
                cliente = value.proveedor_p;
            }
          });
          $("#cliente").val(cliente);
        }
...
function consultaCliente(id, campo) {
  $.ajax({
    data: {
      comprobarCliente: id,
      campo: campo,
    ...
    success: function (data) {
      if (data != 0) {
        ...
        let clientes = {};
        object.forEach((element) => {
          clientes[element.id_c] = element.nombre_c;
        });
        ...
        Swal.fire({
          ...
          inputValidator: (value) => {
            return new Promise((resolve) => {
              if (value) {
                ...
                resolve("Debes seleccionar un Cliente");
              }
            });
          },
        }).then((result) => {
          if (result.isConfirmed) {
            ...
            object.forEach((value) => {
              if (value.id_c === clienteSeleccionado) {
                $("#documento").val(value.nit_c),
                  $("#pais").val(value.ciudad_c),
                  $("#contacto").val(value.contacto_c),
                  $("#telefono").val(value.telefono_c),
                  $("#celular").val(value.celular_contacto_c),
                  $("#direccion").val(value.direccion_c),
                  cliente = value.nombre_c;
        ...
        Swal.fire({
          icon: "error",
          title: "Oops...",
          text: "Cliente no existe, Escribelo de nuevo",
        });
      }
    },
    error: function (xhr, status, error) {
      console.log(error);
      swal.fire("Cliente no existe", JSON.parse(data));
    },
  });
}
