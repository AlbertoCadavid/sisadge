...
function numeracionChar($carac) {
    ...
    if ($codigo1 != null) {
        ...
        preg_match_all('/\D+/', $caract, $matches);
        $cadena = $matches[0];
        preg_match_all('/\d+/', $caract, $matches);
        $solonumeros = $matches[0];

        $cerosizq = cerosIzquierda($caract, $solonumeros, $solonumeros); 
        if ($cerosizq != null) {
            $solonumeros = $cerosizq;
        }
        return [$solonumeros, $cadena];
    }
}
...
function cerosIzquierda($conletras, $solonumero, $cuantos) {
    $ceros = preg_match('/EM|MQ|AB|AC|BP|BM|C|BB|OA|BC/i', $conletras); // códigos especiales
    $cuantos = strlen($cuantos);
    $codigo = str_pad($solonumero, $cuantos, "0", STR_PAD_LEFT);
    return $codigo; // si es cero es porque lo encuentra
}

function buscaDigitos($caract) {
    $codigo = explode("-", $caract); // hasta el guion
    if (isset($codigo[1])) {
        return $codigo;
    }
    return null; // Retorna null si no hay guion
}
?>