<br />
<table width="450" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="10"><img src="imagenes/ventanaAyudaLSI.png" width="21" height="35" alt="" /></td>
    <td style="background-image:url(imagenes/ventanaAyudaCS.png)" ><strong>Estadisticas de Exportacion {hojaDeCalculo}</strong></td>
    <td width="10"><img src="imagenes/ventanaAyudaLSD.png" width="21" height="35" alt="" /></td>
  </tr>
  <tr>
    <td style="background-image:url(imagenes/ventanaAyudaLMI.png);">&nbsp;</td>
    <td style="background-image:url(imagenes/ventanaAyudaFondo.png);">
    <table width="700" border="0" align="center" cellpadding="0" cellspacing="5" class="bordeTablaRedondeado">
  <tr>
    <td width="15">&nbsp;</td>
    <td width="223" class="titulosInternos"><div align="center" >Estado</div></td>
    <td width="223" class="titulosInternos"><div align="center" >Cantidad</div></td>
    <td width="223" class="titulosInternos"><div align="center" >Porcentaje</div></td>
    <td width="15">&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td >&nbsp;</td>
    <td>&nbsp;</td>
    <td >&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  
  <!-- BEGIN errores -->
  <tr>
    <td>&nbsp;</td>
    <td >{errores.numero}</td>
    <td ><div align="right">{errores.cantidad}</div></td>
    <td ><div align="right">{errores.porcentaje}</div></td>
    <td>&nbsp;</td>
  </tr>
  <!-- END errores -->

  <tr>
    <td>&nbsp;</td>
    <td >&nbsp;</td>
    <td>&nbsp;</td>
    <td >&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td >Tiempo de Exportacion:</td>
    <td colspan="2"><div align="center">{tiempoExportacion}</div></td>
    <td>&nbsp;</td>
  </tr>
</table>
    
    </td>
    <td style="background-image:url(imagenes/ventanaAyudaLMD.png);" >&nbsp;</td>
  </tr>
  <tr>
    <td><img src="imagenes/ventanaAyudaLII.png" width="21" height="19" alt="" /></td>
    <td style="background-image:url(imagenes/ventanaAyudaCI.png);">&nbsp;</td>
    <td><img src="imagenes/ventanaAyudaLID.png" width="21" height="19" alt="" /></td>
  </tr>
</table>
