...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  mysql_select_db($database_conexion1, $conexion1);
  $Result2 = mysql_query($insertSQL2, $conexion1) ;
...
if($colname_item_oc!='') 
$row_ref_items = $conexion->llenarCampos("tbl_orden_compra oc,tbl_items_ordenc oci", "WHERE oc.str_numero_oc='".$colname_item_oc."' AND oc.id_pedido=oci.id_pedido_io AND oc.b_borrado_oc='0'  ", "ORDER BY  CAST(oci.int_cod_ref_io AS int) DESC ", " oc.str_nit_oc, oc.id_c_oc, oc.b_borrado_oc,oc.str_numero_oc,oci.str_numero_io,oci.int_cod_ref_io ");

if($colname_item_oc!='') 
$row_ref = $conexion->llenaSelect('tbl_orden_compra oc,tbl_items_ordenc oci', "WHERE oc.str_numero_oc='".$colname_item_oc."' AND oc.id_pedido=oci.id_pedido_io AND oc.b_borrado_oc='0' ","ORDER BY  CAST(oci.int_cod_ref_io AS int) DESC");


//$row_oc = $conexion->llenaSelect("tbl_orden_compra", "WHERE b_estado_oc='1' AND b_borrado_oc='0' ","ORDER BY str_numero_oc DESC");

$row_oc = $conexion->llenaListas('tbl_orden_compra',"WHERE (b_estado_oc <= 1 || b_estado_oc ='') AND  b_borrado_oc='0' AND (proforma_oc is null OR proforma_oc='')  ","ORDER BY str_numero_oc DESC ",'str_numero_oc');  

if($_GET['str_numero_oc_op']!='' && $_GET['int_cod_ref_op']) 
$row_precio = $conexion->llenarCampos("tbl_items_ordenc oci", "WHERE oci.str_numero_io='".$_GET['str_numero_oc_op']."' AND oci.int_cod_ref_io='".$_GET['int_cod_ref_op']."'  ", "  ", " oci.int_precio_io ");

if($_GET['str_numero_oc_op']!='' && $_GET['int_cod_ref_op'])
$row_datos_oc = $conexion->llenarCampos("Tbl_orden_compra,Tbl_items_ordenc,Tbl_referencia,Tbl_egp", "WHERE Tbl_orden_compra.str_numero_oc='".$_GET['str_numero_oc_op']."' AND Tbl_orden_compra.str_numero_oc=Tbl_items_ordenc.str_numero_io AND Tbl_items_ordenc.int_cod_ref_io='".$_GET['int_cod_ref_op']."' AND Tbl_items_ordenc.int_cod_ref_io=Tbl_referencia.cod_ref AND Tbl_referencia.n_egp_ref=Tbl_egp.n_egp AND Tbl_referencia.estado_ref='1'  ", " ORDER BY Tbl_referencia.cod_ref DESC ", " * ");
...
$nitc=$row_ref_items['str_nit_oc'];
 

$row_cotiz = $conexion->llenarCampos("tbl_cotiza_bolsa  "," WHERE N_referencia_c='".$refop."' and Str_nit='".$nitc."' ORDER BY N_cotizacion DESC ", "","B_numeracion"); 
...
function funcion(){ 
if(form1.mensajeLickRef.value=='1' && form1.tipoLamina.value=='1'){ 
swal("La referencia no tiene formula o esta incompleta, debe crearla y Debe agregar el tipo de lamina para bolsillo en la referencia");
return false;
} else
if(form1.mensajeLickRef.value=='1' && form1.tipoLamina.value!='1')
{  
swal("La referencia no tiene formula o esta incompleta, debe crearla");
return false; 
  } else 
if(form1.mensajeLickRef.value!='1' && form1.tipoLamina.value=='1')
{  
swal("Debe agregar el tipo de lamina para bolsillo en la referencia");
return false;
}
  return true;
 }
                 ...
                 <div class="panel-body"> 
                     ...
                     <tr><td id="nombreusuario"><?php echo $_SESSION['Usuario']; ?></td>
                       ...
                       <form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form1" id="form1" onsubmit="return (metrosAkilos() && funcion() && validacion_tipocinta());">
                         <table id="tabla2">
                           <tr id="tr1">
                             <td colspan="12" id="titulo2">ORDEN DE PRODUCCI&Oacute;N</td>
                             </tr>
                           <tr>
                           ...
                           if($_GET['int_cod_ref_op']!=''){          
                             ...
                             if($resultm['cod_ref'] =='' ){
                            ?> 
                               <a href="view_index.php?c=cmezclas&a=Mezcla&cod_ref=<?php echo $cod_ref;?>" target="_blank" style="text-decoration:none; color:#000000"><strong class="rojo_inteso">Agregar la formula</strong></a>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="titulo4"></td>
                             </tr>
                     <tr id="tr1">
                             <td colspan="12" id="titulo4">ESPECIFICACIONES</td>
                             </tr>  
                             <tr id="tr1">
                             <td colspan="12" id="fuente1">
                             <textarea name="observacion5_egp" style="width: 100%" rows="2" id="observacion5_egp" ><?php echo $row_datos_oc['observacion5_egp']; ?></textarea>
                             OBSERVACIONE DEL EGP</td>
                             </tr>      
                           <tr>
                   ...
                   <td id="fuente1">
                    <input type="hidden" name="str_nit_op" id="str_nit_op"  value="<?php echo $row_ref_items['str_nit_oc']; ?>"/>
                       ...
                       { 
                        ...
                        $ca = $nombre_c; echo $ca; 
                        $nit_oc=mysql_result($resultn,0,'nit_c'); 
                      } else { echo "Cliente !";  } ?>
                     </td>
                    <td colspan="2" id="fuente1">
                      <select name="str_numero_oc_op" autofocus id="str_numero_oc_op" style="width:140px" onchange="if(form1.str_numero_oc_op.value) { consulta_oc_op(); } else{ alert('Debe Seleccionar una O.C'); }"  class="busqueda" >
                         <option value="0"<?php if (!(strcmp("", $_GET['str_numero_oc_op']))) {echo "selected=\"selected\"";} ?>>Orde de Compra</option>
                          <?php  foreach($row_oc as $row_oc ) { ?>
                    ...
                    <td  nowrap="nowrap" id="fuente1">
                      ...
                      <select name="int_cod_ref_op" id="int_cod_ref_op" class="selectsMini" onchange="if(form1.int_cod_ref_op.value) { consulta_ref_op() } else{ alert('Debe Seleccionar una REFERENCIA'); }" >
                         <option value=""<?php if (!(strcmp("", $_GET['int_cod_ref_op']))) {echo "selected=\"selected\"";} ?>>Referencia</option>
                          <?php  foreach($row_ref as $row_ref ) { ?>
                         <option value="<?php echo $row_ref['int_cod_ref_io']; ?>"<?php if (!(strcmp($row_ref['int_cod_ref_io'], $_GET['int_cod_ref_op']))) {echo "selected=\"selected\"";} ?>><?php echo $row_ref['int_cod_ref_io']; ?> </option>
                      <?php } ?>
                      </select> 
                               -
                       <input name="version_ref_op" type="number" id="version_ref_op" min="0" max="9" size="2" value="<?php echo $row_datos_oc['version_ref']; ?>" readonly="readonly" required="required" /></td>
                             ...
                             <a href="javascript:verFoto('control_tablas.php?n_cotiz=<?php echo $row_datos_oc['n_cotiz_ref']; ?>&cod_ref=<?php echo $_GET['int_cod_ref_op']; ?>&Str_nit=<?php echo $nit_oc; ?>&case=<?php echo "6"; ?>','850','850')"><em><?php echo $row_datos_oc['n_cotiz_ref']; ?></em></a>
                           </td>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="titulo4">CONDICIONES DE FABRICACI&Oacute;N EN EXTRUSI&Oacute;N </td>
                           </tr>
                           <tr>
                           ...
                           <tr>
                             <td nowrap="nowrap" id="talla1">
                               ...
                               <input id="int_desperdicio_op" name="int_desperdicio_op" type="text" value="0" min="0" max="50"  style="width:40px" required="required" onchange="calcular_op();validacion_tipocinta()"/> %<strong></strong>
                           ...
                           <td id="talla1">  
                             <select name="sinnumeracion" id="sinnumeracion">
                               <option value="0"<?php if (!(strcmp("1", $row_cotiz['B_numeracion']))) {
                                                                echo "selected=\"selected\"";
                                                              } ?>>NO</option>
                               <option value="1"<?php if (!(strcmp("0", $row_cotiz['B_numeracion']))) {
                                                                echo "selected=\"selected\"";
                                                              } ?>>SI</option>
                             </select>
                           </td>
                           ...
                           <tr>
                             ...
                             <td colspan="2" id="fuente1"><input id="int_ancho_rollo_op" name="int_ancho_rollo_op" style="width:70px" type="number" min="0"max="500" size="5" step="0.01" required="required" value="<?php echo $row_datos_oc['ancho_rollo'];?>" onblur="calcular_op();"/></td>
                           </tr>
                           <tr>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="titulo1">Observacion en Extrusion</td>
                             </tr>      
                           <tr>
                             <td id="fuente1"></td>
                             <td colspan="2" id="fuente1"></td>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="fuente1"><textarea name="observ_extru_op" style="width: 100%" rows="3"></textarea></td>
                             </tr>
                           <tr id="tr1">
                             <td colspan="12" id="titulo4">CONDICIONES DE FABRICACI&Oacute;N EN IMPRESION</td>
                             </tr>
                             <tr>
                           ...
                           <tr>  
                             ...
                             <td id="fuente1"><input name="margen_izq_imp_op" id="margen_izq_imp_op" style="width:50px" type="number" min="0" size="5" step="1" value="<?php echo $row_datos_oc['margen_izq_imp_egp'];?>"/></td>
                             <td id="talla1">Rep. en Ancho</td>
                             <td id="fuente1"><input name="margen_anc_imp_op" id="margen_anc_imp_op" style="width:50px" type="number" min="0" size="5" step="1" value="<?php echo $row_datos_oc['margen_anc_imp_egp']?>"/></td>
                             <td colspan="2" id="talla2">de</td>
                             <td id="fuente1"><input name="margen_anc_mm_imp_op" id="margen_anc_mm_imp_op" style="width:50px" type="number" min="0" size="5" step="1" value="<?php echo $row_datos_oc['margen_anc_mm_imp_egp']?>"/></td>
                             <td id="talla1">mm</td>
                           </tr>
                           <tr>
                             <td id="talla1">Derecha mm</td>
                             <td id="fuente1"><input name="margen_der_imp_op" id="margen_der_imp_op" style="width:50px" type="number" min="0" size="5" step="1" value="<?php echo $row_datos_oc['margen_der_imp_egp']?>"/></td>
                             <td id="talla1">Rep. Perimetro</td>
                             <td id="fuente1"><input name="margen_peri_imp_op" id="margen_peri_imp_op" style="width:50px" type="number" min="0" size="5" step="1" value="<?php echo $row_datos_oc['margen_peri_imp_egp']?>"/></td>
                             <td colspan="2" id="talla2">de</td>
                             <td id="fuente1"><input name="margen_per_mm_imp_op" id="margen_per_mm_imp_op" style="width:50px" type="number" min="0" size="5" step="1" value="<?php echo $row_datos_oc['margen_per_mm_imp_egp']?>"/></td>
                             <td id="talla1">mm</td>
                           </tr>
                           <tr>
                             <td id="talla1">&nbsp;</td>
                             <td id="talla1"><strong>Z</strong></td>
                             <td id="fuente1"><input name="margen_z_imp_op" id="margen_z_imp_op" style="width:50px" type="number" min="0" step="0.01" value="<?php echo $row_datos_oc['margen_z_imp_egp']?>"/></td>
                             <td colspan="6" id="talla1">&nbsp;</td>
                             </tr>
                           <!-- <tr>  
                             <td colspan="12"> 
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="titulo1">Observacion en Impresion</td>
                             </tr>      
                           <tr>
                             <td id="fuente1"></td>
                             <td colspan="2" id="fuente1"></td>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="fuente1"><textarea name="observ_impre_op" style="width: 100%" rows="3" id="observ_impre_op"></textarea></td>
                             </tr>
                           <tr>
                             <td colspan="12" id="fuente5">&nbsp;</td>
                             </tr>
                                     <!--SELLADO NO APARECE SI ES LAMINA EXCEPTO UND X CAJAS Y PAQUETES-->
                                     
                             <?php //if ((strcmp("LAMINA", $row_datos_oc['tipo_bolsa_ref']))) { ?>
                     <tr id="tr1">
                             <td colspan="12" id="titulo4">CONDICIONES DE FABRICACI&Oacute;N EN SELLADO</td>
                             </tr>        
                       ...
                       <tr>
                         ...
                         <td id="talla1">&nbsp;</td>
                         <td id="talla1"><?php if($row_datos_oc['bolsillo_guia_ref']!='0.00' && $row_datos_oc['tipoLamina_ref']=='0') {?> <input name="tipoLamina" type="hidden" value="1"/><?php }else {
                       ...
                       <tr>
                         ...
                         <td id="talla1"><input name="undxpaqreal" style="width:50px" type="number" id="undxpaqreal" value="<?php echo $row_ultima_numeraciones['undxpaqreal']; ?>" required="required" size="12" min="0" /></td>
                         <td id="talla1">
                        ...
                        <td id="talla1"><input name="lote" style="width:70px" type="text" id="lote" value="<?php echo $op_numero; ?>" placeholder="Lote" /></td>
                         <td id="talla1"> 
                         <input name="numInicio_op" type="text" id="numInicio_op" style="width:80px" min="0" step="1" value="<?php echo $row_numeraciones['int_hasta_tn']!=''? $row_numeraciones['int_hasta_tn'] : "0"; ?>" required="required" onBlur="hastaordenP();" /><input class="charfin" style=" width:40px" type="text" name="charfin" autofocus id="charfin" value="" onchange="conMayusculas(this)" > 
                       ...
                       <tr>
                         ...
                         <td id="talla1"><input name="und_prod_sellado_op" type="number" id="und_prod_sellado_op" style="width:70px" min="0" step="0.01" value="<?php echo $row_datos_oc['int_cantidad_io']?>"/></td>
                     ...
                     <!--      <select name="id_termica_op" id="id_termica_op" style="width:100px">
                        ...
                        <td id="talla1"><input type="number" name="cinta_termica_op" id="cinta_termica_op" value="<?php echo $row_datos_oc['cinta_termica_ref'];?>" style="width:50px" min="0" step="0.01" required="required"/>
                         </td> 
                        </tr>
                      </table>
                             </td>
                           ...
                           <tr>
                             <td colspan="12" id="talla1"><strong>Nota:</strong> Si en el campo de la version de la referencia no coninciden es porque no han hecho la verificacion u modificacion en dise&ntilde;o y desarrollo.</td>
                           </tr>     
                           <tr id="tr1">
                             <td colspan="12" id="titulo1">Observacion en Sellado</td>
                             </tr>      
                           <tr>
                             <td id="fuente1"></td>
                             <td colspan="2" id="fuente1"></td>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="fuente1"><textarea name="observ_sellado_op" style="width: 100%" rows="3" id="observ_sellado_op"></textarea></td>
                             </tr>
                             <?php //}?>
                           <tr>
                           ...
                           <tr id="tr1">
                             <td colspan="12" id="dato2"><input name="id_proceso" type="hidden" value="1" />
            ...
            ajax: {
                ...
                data: function (params) {
                    return {
                        ...
                        var3:" b_estado_oc='1' AND b_borrado_oc='0' ",//where
      ...
      ajax: {
          ...
          data: function (params) {
              return {
                  ...
                  var3:" (b_estado_oc <= 1 || b_estado_oc ='') AND  b_borrado_oc='0' AND (proforma_oc is null OR proforma_oc='') ",//where
                  var4:" ORDER BY fecha_ingreso_oc DESC",
