...
<?php foreach ($this->row_control_paquete as $row_control_paquete) {
  $row_control_paquete;
} ?>
<?php //foreach($this->row_op as $row_op) { $row_op;} 
?>
<?php foreach ($this->row_codigo_empleado as $row_codigo_empleado) {
  $row_codigo_empleado;
} ?>
<?php foreach ($this->row_revisor as $row_revisor) {
  $row_revisor;
} ?>
<?php foreach ($this->row_tiquete_num as $row_tiquete_num) {
  $row_tiquete_num;
} ?>
<?php foreach ($this->existe_caja as $existe_caja) {
  $existe_caja;
} ?>
<?php foreach ($this->select_tiquete_num as $select_tiquete_num) {
  $select_tiquete_num;
} ?>
                          ...
                          <tr>
                            <td colspan="3">
                              ...
                              <input <?php if ($restrincion != '1') {
                                        echo "readonly";
                                      } ?> class="form-control negro_inteso" name="int_paquete_tn" style="width:100px;" type="number" id="int_paquete_tn" value="<?php echo $row_control_paquete['int_paquete_tn']; ?>" maxlength="5">
                            </td>

                            <td>
                              <input <?php if ($restrincion != '1') {
                                        echo "readonly";
                                      } ?> class="form-control negro_inteso" name="int_caja_tn" id="int_caja_tn" style="width:200px;" type="number" value="<?php echo $select_tiquete_num["int_caja_tn"]; ?>" maxlength="5">
                            </td>
                          ...
                          <tr>
                            ...
                            <td colspan="3" id="fuente1">
                              <select class="form-control" name="int_cod_empleado_tn" id="int_cod_empleado_tn">
                                <option value="" <?php if (!(strcmp("", $row_tiquete_num['int_cod_empleado_n']))) {
                          ...
                          <tr>
                            ...
                            <td colspan="3" id="fuente1">
                              <select class="form-control" name="int_cod_rev_tn" id="int_cod_rev_tn">
                                <option value="" <?php if (!(strcmp("", $row_tiquete_num['int_cod_rev_n']))) {
                                                    echo "selected=\"selected\"";
                                                  } ?>>Seleccione</option>
                            ...
                            <td colspan="6">
                              ...
                              <div id="paqycajasnormal" style="display: none;">
                                <div class="bordesolido" id="ventanas">
                                  ...
                                  <!-- <a href="javascript:popUp('sellado_control_numeracion_vista_colas.php?id_op=<?php //echo $ops;
                                                                                                                    ?>&int_caja_tn=<?php //echo $row_tiquete_num['int_caja_n']; 
                                                                                                                                    ?>','1200','780')" target="_top"><strong class="letraPaquete">IMIPRIMIR PAQUETES POR CAJA</strong></a>  -->
                                </div>
                              </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    valCheck(numDesde);
    numeracionDesde(numDesde, caja, paquete, ref, $("#contador_tn").val()); //cuadra la numeracion inicial y el hasta
  ...
  $("#int_desde_tn").on('change', function() {
    //var ref = "<?php echo $select_tiquete_num['ref_tn'] == '' ? $row_control_paquete['int_cod_ref_op'] : $select_tiquete_num['ref_tn']; ?>";
    numeracionDesde($("#int_desde_tn").val(), $("#int_caja_tn").val(), $("#int_paquete_tn").val(), $("#contador_tn").val());
  });
  ...
  $("#id_rollo").on("change", function() {
    ...
    swal({
        title: "Esta seguro?",
        text: "Seguro que quiere seleccionar el Rollo " + numRollo,
        icon: "warning",
        buttons: true,
        dangerMode: true,
      })
  ...
  function calculoBanderas(datosBanderas) {
    ...
    let mtsBandImpresion = [];
    let arrayBanderasImpresion = [];
