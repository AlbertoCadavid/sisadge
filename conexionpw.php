...
function getVentasbyDay()
{
	...
	//transportadorea viene en jaso _wc_shipment_tracking
	//SI NO CAEN EN SISADGE  es porque no tienen el documento o cedula por eso se valida en estas dos columnas _item _billing_cedula  _billing_wooccm11 
	$conexion = Conectar();
	$sql = "SELECT * FROM (
	SELECT 
	POST_ID AS ORDEN,
	MAX(CASE WHEN META_KEY =  '_billing_cedula' THEN meta_value  END) AS CEDULA,
...
//echo 'Orden: '. $value["ORDEN"] . 'order_item_id: '.$value1["order_item_id"].'<br>' ;die;
			...
			if($cedulanit!='')
			{
               ...
               $conexion->query($insert);
                ...
                $obser_guia =  ($obser_guia);
                	...
                	$resultadonota = $conexion->query("select post_excerpt from wp5s_posts where id =".$cliente['ORDEN']);
                	$productosnota = getResultados($resultadonota);
                	foreach($productosnota as $key1 => $valuenotas)
                	{   
                	    $notaorden = $valuenotas["post_excerpt"];
                	    if($notaorden!=''){

                	       $notaorden = str_replace("'", '', $notaorden);
                	       $notaorden = str_replace(".", '', $notaorden);
                	       $notaorden = str_replace("#", '', $notaorden);

                	    }
                	       if($notaorden==''){
                              $especialweb = 0;//variable  para las ordenes especiales que traen nota y poder filtrar en listado de o.c
                	       }else{
                	       	  $especialweb = 1;
                	       }
                    ...
                    //consulto que no exista una misma o.c para el mismo cliente  
                	$query_ordenc = $conexionpw->query("SELECT str_numero_oc FROM tbl_orden_compra WHERE str_numero_oc='".$cliente['ORDEN'].$codigo_orden."' AND str_nit_oc='".$cedulanit."' ");
                	$result_ordenc[0] = $query_ordenc->fetch_row(); 

                $fecha_autoriza = date('Y-m-d H:i:s'); 
                
       if(!$result_ordenc[0]){

                $insertSQL2 = "INSERT INTO tbl_orden_compra (str_numero_oc,id_c_oc,str_nit_oc,fecha_ingreso_oc,str_condicion_pago_oc,str_observacion_oc,int_total_oc,str_dir_entrega_oc, str_archivo_oc,  str_elaboro_oc,str_aprobo_oc,b_estado_oc,str_responsable_oc,b_borrado_oc,b_facturas_oc,vta_web_oc, entrega_fac, fecha_cierre_fac, comprobante_ent,pago_pendiente,fecha_autoriza,notaweb,especialweb) VALUES ('".$cliente['ORDEN'].$codigo_orden."','".$idcliente."','".$cedulanit."','".$fecha."','".$cliente['METODO_PAGO']."','".$obser_guia."','".$cliente['TOTAL']."','".$dirFacturacion."', 'factura-".$cliente['ORDEN'].".pdf', 'PW','PW','1','PW', '0','1','1','SI','".$fecha."','NO','NO','".$fecha_autoriza."','".$notaorden."','".$especialweb."'  )"; 
                ...
                foreach ($cliente['detalle'] as $key => $itemsoc) 
                {
                    ...
                    if($sku_ref_especial=='1130'){ 
                    		...
                    		$sku_ref = $sku_ref_codigo[1]; 
                    		$precioUnidad = "PRECIO COMBOS";
                    }else if($sku_ref_especial=='CPL'){ 
                    		$unidades = 1;//producto combo
                    		$sku_ref_codigo = explode("CPL", $sku_ref_especial);
                    		$sku_ref = $sku_ref_codigo[1];
                	...
                	$conexionpw->query($insertSQL4);
                      ...
                      if($cotiz_id[0]){
                      	//INFO DE REF
                      	$query_ref = $conexionpw->query("SELECT ancho_ref,largo_ref,solapa_ref,n_fuelle,calibre_ref,bolsillo_guia_ref,tipo_bolsa_ref,b_solapa_caract_ref,sello_superior FROM Tbl_referencia WHERE cod_ref='".$sku_ref."' ");
                        ...
                        //se relaciona referencia 
                        if($especialweb==1){
                           
                            $arrayName = array('1253','1254','1256','1257');//1254 / 1256 / 2157 / 1253 se relacionan al cliente si este tienes notas especiales
                               foreach ($arrayName as $refweb) {
                               	 
                        	      $refcliente = "INSERT INTO tbl_cliente_referencia (N_referencia, N_cotizacion, Str_nit) VALUES('".$refweb."','".$cotiz_id[0]."','".$cedulanit."' )";    
                      	          $conexionpw->query($refcliente); 
                      ...
                      if($result_ref[6]=='PACKING LIST'){
                        ...
                        $conexionpw->query($insertCotiz); 
                         ...
                         $insertCotiz = "INSERT INTO tbl_cotiza_laminas (N_cotizacion, N_referencia_c, Str_nit, Str_moneda, N_precio_k, Str_unidad_vta, N_cant_impresion, fecha_creacion, Str_usuario, B_estado,B_generica,N_ancho,N_alto,N_solapa,B_fuelle,N_calibre,N_tamano_bolsillo,N_precio_old,valor_impuesto) VALUES (".$cotiz_id[0].",'".$sku_ref."', '".$cedulanit."', 'COL$', '".$precioOld."','".$precioUnidad."','".$cantidadItem."','".$fecha."', '65', '1', '1','".$result_ref[0]."','".$result_ref[1]."','".$result_ref[2]."','".$result_ref[3]."','".$result_ref[4]."','".$result_ref[5]."','".$valorunidad."', '".$precioImpuesto."' )";    
                      	...
                      	$insertCotiz = "INSERT INTO tbl_cotiza_bolsa (N_cotizacion, N_referencia_c, Str_nit, Str_moneda, N_precio, Str_unidad_vta, N_cant_impresion, fecha_creacion, Str_usuario, B_estado,B_generica,tipo_bolsa,tiposolapa,sello_superior,N_ancho,N_alto,N_solapa,B_fuelle,N_calibre,N_tamano_bolsillo,N_precio_old,valor_impuesto) VALUES (".$cotiz_id[0].",'".$sku_ref."', '".$cedulanit."', 'COL$', '".$precioOld."','".$precioUnidad."','".$cantidadItem."','".$fecha."', '65', '1', '1','$result_ref[6]','$result_ref[7]','$result_ref[8]', '".$result_ref[0]."','".$result_ref[1]."','".$result_ref[2]."','".$result_ref[3]."','".$result_ref[4]."','".$result_ref[5]."','".$valorunidad."', '".$precioImpuesto."' )";    
                      	$conexionpw->query($insertCotiz); 
                   ...
                   }//FIN GUARDO COTIZACION
               } 
          } //termina foreach de items de orden c

       }//fin if de si existe o.c 

				  //exit($insertCotizrel);        

   }else{
   	echo '<br>';
   	logs("ORDEN: ".$cliente['ORDEN']. " - Documento Identidad Vacio!");
   	echo '<br>';die;
   }
