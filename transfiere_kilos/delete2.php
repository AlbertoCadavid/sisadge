...
$numerd= mysql_num_rows($resultrd);
if($numerd >='1') {
	$iddesp=mysql_result($resultrd,0,'id_rpd_rd');
	$id_op=mysql_result($resultrd,0,'op_rd');
	$id_rddel=mysql_result($resultrd,0,'id_rd');
	$fecha=mysql_result($resultrd,0,'fecha_rd');	
	$valores=mysql_result($resultrd,0,'valor_desp_rd');
	$rollo_r=mysql_result($resultrd,0,'int_rollo_rd');
	$idproceso=mysql_result($resultrd,0,'id_proceso_rd');
	  ...
	  updateLiquida($iddesp,$rollo_r,$id_op,$valores,$idproceso );
...
header("location:produccion_registro_extrusion_edit.php?id_op=$id_op&id_rp=$id_rp");
 ...
 function updateLiquida($iddesp,$rollo,$ops,$kilos,$idproceso ){
   ...
   foreach($extruder as &$valor) { 
        if($valor == $iddesp){

          updateRollos("tbl_reg_produccion",$idproceso,$rollo,$ops,$kilos);//se modifica la liquidacion 
         
        } 
   }
   foreach($impresion as &$valor2) { 
        if($valor2 == $iddesp){ 
...
function updateRollos($tabla,$proceso_modificar,$rollo,$ops,$kilos){

    //Metros correspondientes a ese desperdicio
    $nuevometros=operacionesGenmetros("TblExtruderRollo" ," WHERE id_op_r = '$ops' AND rollo_r='$rollo'","", $kilos);
    $nuevometros2=operacionesGenmetros("tblimpresionrollo" ," WHERE id_op_r = '$ops' AND rollo_r='$rollo'","", $kilos);
    $nuevometros3=operacionesGenmetros("tblselladorollo" ," WHERE id_op_r = '$ops' AND rollo_r='$rollo'","", $kilos);

    $extruderRollos=updateRollosProceso($ops,$rollo,$nuevometros,$kilos,$proceso_modificar);
    

     if($proceso_modificar=='1'){
       
      $impresion=("UPDATE $tabla SET int_kilos_prod_rp = int_kilos_prod_rp + '$kilos', int_total_kilos_rp=int_total_kilos_rp+'$kilos', int_metro_lineal_rp=int_metro_lineal_rp+'$nuevometros2'  WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='2'");//Impresion
      $result2= mysql_query($impresion);

      $sellado=("UPDATE $tabla SET int_kilos_prod_rp = int_kilos_prod_rp + '$kilos', int_total_kilos_rp=int_total_kilos_rp+'$kilos', int_metro_lineal_rp=int_metro_lineal_rp+'$nuevometros3' WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='4'");//sellado
      $result3= mysql_query($sellado);

     }else if($proceso_modificar=='2'){
      
      $sellado=("UPDATE $tabla SET int_kilos_prod_rp = int_kilos_prod_rp + '$kilos', int_total_kilos_rp=int_total_kilos_rp+'$kilos', int_metro_lineal_rp=int_metro_lineal_rp+'$nuevometros3' WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='4'");//sellado
      $result2= mysql_query($sellado);

     } 
       $kilosxhora=generalkiloxhora($tabla," WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='1'","int_kilos_prod_rp");
       $kilosxhora2=generalkiloxhora($tabla," WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='2'","int_total_kilos_rp");
       $kilosxhora3=generalkiloxhora($tabla," WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='4'","int_total_kilos_rp");

       $extruderM=("UPDATE $tabla SET int_kilosxhora_rp='$kilosxhora' WHERE id_op_rp='$ops' AND rollo_rp='$rollo' AND id_proceso_rp='1'");
       $result1= mysql_query($extruderM);
       $impresionM=("UPDATE $tabla SET int_kilosxhora_rp='$kilosxhora2' WHERE id_op_rp='$ops' AND rollo_rp='$rollo' AND id_proceso_rp='2'");
       $result2= mysql_query($impresionM);
       $selladoM=("UPDATE $tabla SET int_kilosxhora_rp='$kilosxhora3' WHERE id_op_rp='$ops' AND rollo_rp='$rollo' AND id_proceso_rp='4'");
       $result3= mysql_query($selladoM); 
 
       $metroxmin=generalMetrosxmin($tabla," WHERE id_op_rp = '$ops' AND rollo_rp='$rollo' AND id_proceso_rp='2' " );
       $impresionMxM=("UPDATE $tabla SET int_metroxmin_rp='$metroxmin' WHERE id_op_rp='$ops' AND rollo_rp='$rollo' AND id_proceso_rp='2'");
       $result4= mysql_query($impresionMxM);


   

}
 function updateRollosProceso($ops,$rollo,$nuevometros,$kilos,$proceso){
          
 try 
   { 
   
   if($proceso=='1'){
    $impresion=("UPDATE tblimpresionrollo SET kilos_r = kilos_r  + '$kilos', metro_r=metro_r+'$nuevometros' WHERE id_op_r = '$ops' AND rollo_r='$rollo' "); 
    $result1= mysql_query($impresion);
    $sellado=("UPDATE tblselladorollo SET kilos_r = kilos_r  + '$kilos', metro_r=metro_r+'$nuevometros' WHERE id_op_r = '$ops' AND rollo_r='$rollo' ");
    $result2= mysql_query($sellado);
   }
    
  if($proceso=='2'){
    $sellado=("UPDATE tblselladorollo SET kilos_r = kilos_r  + '$kilos', metro_r=metro_r+'$nuevometros' WHERE id_op_r = '$ops' AND rollo_r='$rollo' ");
    $result1= mysql_query($sellado);
  }
...
function generalkiloxhora($tabla,$condicion,$columna){
    $sqlID = ("SELECT ($columna) as total_kilos,total_horas_rp FROM $tabla $condicion") ;
    $resultID= mysql_query($sqlID);
    $numeID= mysql_num_rows($resultID);

    	if($numeID >='1') {
    		  $kilos2=mysql_result($resultID,0,'total_kilos');
          $horas2=mysql_result($resultID,0,'total_horas_rp');
...
function generalMetrosxmin($tabla,$condicion ){
  $sqlID = ("SELECT total_horas_rp,int_metro_lineal_rp FROM $tabla $condicion ") ;
  $resultID= mysql_query($sqlID);
    $numeID= mysql_num_rows($resultID);

 if($numeID >='1') {
  
  $metros=mysql_result($resultID,0,'int_metro_lineal_rp');
  $horas=mysql_result($resultID,0,'total_horas_rp'); 
  ...
  function operacionesGenmetros($tabla,$condicion,$order='',$desp){
    
     
      	$sqlID="SELECT kilos_r,metro_r FROM $tabla $condicion $order  ";
      	$resultID= mysql_query($sqlID);
      	$numeID= mysql_num_rows($resultID);
      	if($numeID >='1') {
      		  $valor_kilos=mysql_result($resultID,0,'kilos_r');
            $valor_lineal=mysql_result($resultID,0,'metro_r');

            $kilosTotales=($valor_kilos+$desp);
            $metrosfinal = round(($desp * $valor_lineal) / $valor_kilos,0);
