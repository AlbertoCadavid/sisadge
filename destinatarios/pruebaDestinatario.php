...
/*  $insertSQL = sprintf("INSERT INTO cotizacion (n_cotiz, id_c_cotiz, responsable_cotiz, fecha_cotiz, hora_cotiz) VALUES (%s, %s, %s, %s, %s)",
 ...
 $insertSQL = sprintf("INSERT INTO Tbl_Destinatarios (nit, nombre_responsable, direccion, telefono, ciudad ) VALUES (%s, %s, %s, %s, %s)",
  ...
  $insertGoTo = "consulta_destinatario.php?nit=" . $_POST['nit'] . "&nit=" . $_POST['nit'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//CONSULTA PARA DESTINATARIOS
mysql_select_db($database_conexion1, $conexion1);
$query_destinatarios = "SELECT * FROM Tbl_Destinatarios ORDER BY nombre_responsable ASC";
$destinatarios = mysql_query($query_destinatarios, $conexion1) or die(mysql_error());
$row_destinatarios = mysql_fetch_assoc($destinatarios);
$totalRows_destinatarios = mysql_num_rows($destinatarios);
?>
...
<tr><td id="nombreusuario"><?php echo $row_usuario['nombre_usuario']; ?></td>
  ...
  <tr>
	...
	<form action="<?php echo $editFormAction; ?>" method="post" name="form1" onSubmit="MM_validateForm('nit','','R');return document.MM_returnValue">
  <table id="tabla2">
    <tr id="tr1">
    ...
    <tr>
      <td colspan="2" id="dato1"><select name="nit" id="nit" onBlur="DatosGestiones('1','nit',form1.nit.value);" >
          <option value="">*</option>
          <?php
do {  
?>
          <option value="<?php echo $row_destinatarios['nit']?>"><?php echo $row_destinatarios['nombre_responsable']?></option>
          <?php
} while ($row_destinatarios = mysql_fetch_assoc($destinatarios));
  $rows = mysql_num_rows($destinatarios);
  if($rows > 0) {
      mysql_data_seek($destinatarios, 0);
	  $row_destinatarios = mysql_fetch_assoc($destinatarios);
  }
?>
      </select></td>
...
mysql_free_result($destinatarios);
