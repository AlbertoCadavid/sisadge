...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (
      stristr($param, "pageNum_numeracion") == false &&
...
<body>
  ...
  <form action="despacho_faltantes2.php" method="GET" name="form1" id="form1">
      ...
      <tr>
        <td colspan="5" id="titulo2">REF/O.C/O.P
          <select class="busqueda selectsGrande" name="id_op" id="id_op" required="required">
            <option value="" <?php if (!(strcmp("", $_GET['id_op']))) {
                                echo "selected=\"selected\"";
                              } ?>>Seleccione REF / O.C / O.P</option>
            <?php foreach ($row_op as $row_op) { ?>
              <option value="<?php echo $row_op['id_op'] ?>" <?php if (!(strcmp($row_op['id_op'], $_GET['id_op']))) {
                                                                echo "selected=\"selected\"";
                                                              } ?>><?php echo $row_op['int_cod_ref_op'] . " / O.C: " . $row_op['str_numero_oc_op'] . " / O.P: " . $row_op['id_op'] ?></option>
            <?php } ?>
          </select>
  ...
  <form id="form2" name="form2" action="despacho_faltante_excel.php" method="POST">
    ...
    <div style="height:600px;width:990px;overflow:scroll;">
      <table class="table">
        ...
        <?php foreach ($registros as $row_numeracion) {  ?>
          ...
          $query_total_faltantes = "SELECT count(int_total_f) as cantidad, sum(int_total_f) as total FROM Tbl_faltantes WHERE Tbl_faltantes.id_op_f= '$opF'AND Tbl_faltantes.int_paquete_f='$paqF' AND Tbl_faltantes.int_caja_f='$cajaF'";
      
      $vista_total_faltantes = mysql_query($query_total_faltantes, $conexion1) or die(mysql_error());
      $row_vista_total_faltantes = mysql_fetch_assoc($vista_total_faltantes);
          ...
          ?>
            <tr onMouseOver="uno(this,'CBCBE4');" onMouseOut="dos(this,'#FFFFFF');" bgcolor="#FFFFFF">
              ...
              <td id="dato2" class="op<?php echo $count ?>"> <input type="hidden" id="op[]" name="op[]"value="<?php echo $row_numeracion['int_op_tn']; ?>"> <?php echo $row_numeracion['int_op_tn']; ?></td>
              <td id="dato2" class="ref<?php echo $count ?>"> <input type="hidden" id="ref[]" name="ref[]" value="<?php echo $row_numeracion['ref_tn']; ?>"><?php echo $row_numeracion['ref_tn']; ?></td>
              <td id="dato2" class="caja<?php echo $count ?>"> <input type="hidden" id="caja[]" name="caja[]" value="<?php echo $row_numeracion['int_caja_tn']; ?>">
              ...
              <td id="dato2" class="undCaja<?php echo $count ?>"> <input type="hidden" id="undCaja[]" name="undCaja[]" value="<?php echo $row_numeracion['int_undxcaja_tn']; ?>"> <?php echo $row_numeracion['int_undxcaja_tn']; ?></td>
              <td nowrap="nowrap" id="dato2" class="desde<?php echo $count ?>"><input type="hidden" id="desde[]" name="desde[]" value="<?php echo $row_numeracion['int_desde_tn']; ?>"><?php echo $row_numeracion['int_desde_tn']; ?><?php echo $row_info_op['charfin']; ?></td>
              <td nowrap="nowrap" id="dato2" class="hasta<?php echo $count ?>"><input type="hidden" id="hasta[]" name="hasta[]" value="<?php echo $row_numeracion['int_hasta_tn']; ?>"> <?php echo $row_numeracion['int_hasta_tn']; ?><?php echo $row_info_op['charfin']; ?></td>
              <td id="dato2" nowrap="nowrap" class="peso<?php echo $count ?>"><input type="hidden" id="peso[]" name="peso[]" value="<?php echo $row_numeracion['pesot']; ?>"> <?php echo $row_numeracion['pesot']; ?></td>
              <td id="dato2" class="paquete<?php echo $count ?>"> <input type="hidden" id="paquete[]" name="paquete[]" value="<?php echo $row_numeracion['int_paquete_tn']; ?>">
                <?php
            ...
            $row_vista_faltantes=mysql_fetch_assoc($resultfal); */
              ...
              <td id="dato2" class="undPaquete<?php echo $count ?>"><input type="hidden" id="undPaquete[]" name="undPaquete[]" value="<?php echo $row_numeracion['int_undxpaq_tn']; ?>"><?php echo $row_numeracion['int_undxpaq_tn']; ?></td>


              <td nowrap="nowrap" id="dato2" class="falDesde<?php echo $count ?>"> <input type="hidden" id="falDesde[]" name="falDesde[]" value="<?php echo $row_vista_faltantes['int_inicial_f']; ?>">
                <?php echo $row_vista_faltantes['int_inicial_f']; ?><?php if ($row_vista_faltantes['int_inicial_f'] != '') {
                                                                      echo $row_info_op['charfin'];
                                                                    } ?>

              </td>
              <td nowrap="nowrap" id="dato2" class="falHasta<?php echo $count ?>"> <input type="hidden" id="falHasta[]" name="falHasta[]" value="<?php echo $row_vista_faltantes['int_final_f']; ?>">
                <?php echo  $row_vista_faltantes['int_final_f']; ?><?php if ($row_vista_faltantes['int_final_f'] != '') {
                                                                      echo $row_info_op['charfin'];
                                                                    } ?>

              </td>
              <td id="dato2" class="undFaltante<?php echo $count ?>"> <input type="hidden" id="undFaltante[]" name="undFaltante[]" value="<?php echo $row_vista_faltantes['int_total_f']; ?>"><?php echo  $row_vista_faltantes['int_total_f']; ?></td>
              

              <td id="dato2" > <input class="totalFaltantes<?php echo $count ?>" type="hidden" value="<?php echo $row_vista_total_faltantes['total'];?>"></td>
...
<script>
  ...
  function getInfo() {
    ...
    let windowExcel = window.open('despacho_faltante_excel.php', '_blank', 'width=1,height=1,left=9999,top=9999');
