...
<title>Demo Sistema Inventario</title>
...
//iniciamos consulta para seleccionar el total en bodega
/*$query_total = ("SELECT * FROM mov_servicios WHERE serv_tipo = '".$criterio."'");
$total = mysql_query($query_total) or die ( "Error en query: $sql, el error  es: " . mysql_error() );//(mysql_error());
$row_total = mysql_fetch_assoc($total);
$totalRows_total = mysql_num_rows($total);*/
//terminamos consulta para seleccionar el total en bodega

//comenzamos la consulta de los movimientos de articulos
$query_movimiento = ("SELECT * FROM mov_servicios WHERE serv_tipo = '".$criterio."' AND datediff(now(),serv_fecha)=0 ORDER BY serv_fecha DESC");//mov_fecha > DATE_SUB( CURDATE( ) , INTERVAL DAYOFMONTH( CURDATE( ) )DAY ) ORDER BY mov_fecha DESC");
