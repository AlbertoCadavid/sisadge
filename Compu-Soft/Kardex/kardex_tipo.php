...
<center>
	<form method="POST" action="kardex_tipo2.php" id="formulario" >
	<!-- INICIO SECCION KARDEX DE PARTICULO!-->
<fieldset><legend><b>Kardex Movimientos</b></legend>
  
  <?php echo '<select name="criterio">'.$options_prd.'</select>'; ?>
  <?php //echo '<input type="hidden" name="descripcion" value="'.$prd.'" />'; 
  //$prd
  ?> 
  <input type="submit" name="consulta" value="Ver Kardex" class="dos"/>
  </form>
</fieldset>
<!-- FIN SECCION KARDEX DE ARTICULO !-->
