...
$query_movimiento = ("SELECT * FROM mov_servicios, equipos WHERE modelo = serv_modelo AND datediff(now(),serv_fecha)=0 ORDER BY serv_fecha DESC");//serv_fecha > DATE_SUB(CURDATE(), INTERVAL DAYOFMONTH( CURDATE())DAY) ORDER BY serv_fecha ASC");//mov_fecha > DATE_SUB( CURDATE( ) , INTERVAL DAYOFMONTH( CURDATE( ) )DAY ) ORDER BY mov_fecha DESC");
...
<tr style="background:#FFD700;">
  ...
  <?php do { ?>
    <tr bgcolor='#FFFACD'>
	  ...
	  <td><?php echo $row_movimiento['serv_cli']; ?></td>
	  <td><?php echo $row_movimiento['serv_tel']; ?></td>
