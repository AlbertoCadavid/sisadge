...
if (isset($_POST['clave_antigua']) && isset($_POST['clave_nueva1']) && isset($_POST['clave_nueva2'])) {
  ...
  $LoginRS = $conexion->buscar('usuario', 'email_usuario', $emailUsername);
  $userTrue = password_verify($password, $LoginRS['clave_usuario']);
  
  if ($userTrue) {
    //UPDATE A LA CLAVE DE CONTRASE�A
    $passNew = $_POST['clave_nueva1'];
    $clave = password_hash($passNew, PASSWORD_DEFAULT);
    ...
    function validarForm(form1) {
      ...
      /*else {
        ...
        var nMay = 0,
          nMin = 0,
          nNum = 0
        var t1 = "ABCDEFGHIJKLMN�OPQRSTUVWXYZ"
        var t2 = "abcdefghijklmn�opqrstuvwxyz"
        var t3 = "0123456789"
...
<body oncontextmenu="return false">
  <div class="container-fluid" id="divconten">
        ...
        <form ACTION="<?php echo $loginFormAction; ?>" method="POST" name="form1" onsubmit="return validarForm(this);">
          <ul>
            <li><label for="pswd">Clave Anterior:</label>
              <span><input id="pswd" type="text" name="clave_antigua" required /></span>
            </li>
            <li> <label for="pswd">Clave Nueva:</label>
              <span><input id="pswd" type="password" name="clave_nueva1" required /></span>
            </li>
            <li><label for="pswd">Clave Nueva:</label>
              <span><input id="pswd" type="password" name="clave_nueva2" required /></span>
            </li>
    ...
    <div class="col-md-4">
      <strong>CAMBIO DE CONTRASE�A</strong><br><br>
      El sistema puedes cambiar tu contrase�a, siempre y cuando tengas un correo de la compa�ia.<br><br>
    </div>
  </div>
