	...
	if (count($_GET['borrar'])) {
		foreach ($_GET['borrar'] as $v) {
			...
			if ($num1 >= '1') {
				...
				$id = 1;
				header("location:empleados.php?id=$id");
			} else {
				$id = 0;
				header("location:empleados.php?id=$id");
			}
...
if ($_GET['borrado'] == '21') {
	if (count($_GET['borrar'])) {
		foreach ($_GET['borrar'] as $v) {
			...
			$id = 1;
			header("location:egp_lamina.php?id=$id");
		}
	} else {
		$id = 0;
		header("location:egp_lamina.php?id=$id");
	...
	if (count($_GET['refcliente'])) {
		foreach ($_GET['refcliente'] as $v) {
			...
			if ($numv >= '1') {
				...
				$id = 1;
				header("location:ref_ac_ref_cl_listado.php?id=1");
			} else {
				$id = 0;
...
if ($_GET['borrado'] == '42') {
	if (count($_GET['borrar'])) {
		foreach ($_GET['borrar'] as $v) {
			...
			$resultemp = mysql_query($sqlemp);
			$id = 1;
			header("location:costos_inventario_entrada_listado.php?id=$id");
		}
	} else {
		$id = 0;
		header("location:costos_inventario_entrada_listado.php?id=$id");
...
if ($_GET['borrado'] == '43') {
	if (count($_GET['borrar'])) {
		foreach ($_GET['borrar'] as $v) {
			...
			$id = 1;
			header("location:proceso_ajuste_listado.php?id=$id");
		}
	} else {
		$id = 0;
		header("location:proceso_ajuste_listado.php?id=$id");
...
if ($_GET['borrado'] == '50') {
	if (count($_GET['ref'])) {
		...
		$id = 0;
		header("location:referencia_copia.php?id=$id");
	}
}
?>