...
WHERE a.tipo_empleado IN(5,10) AND b.fecha BETWEEN DATE_FORMAT('$FECHA_NOVEDAD_IMP', '%Y-%m-01') AND DATE_FORMAT('$FECHA_NOVEDAD_IMP', '%Y-%m-31')"; //IN(5,10)novedad impresion 
  ...
  $updateSQL3 = sprintf(
    "UPDATE Tbl_orden_produccion SET b_estado_op='2', f_impresion=DATE(%s) WHERE id_op='%s'",
    GetSQLValueString($_POST['fechaF_r'], "date"),
    GetSQLValueString($_POST['id_op_r'], "int")
  );
  mysql_select_db($database_conexion1, $conexion1);
...
$row_orden_produccion = $conexion->llenarCampos("tbl_orden_produccion ", "WHERE id_op='" . $_GET['id_op_r'] . "' AND b_borrado_op='0' ", "ORDER BY id_op DESC", " int_cod_ref_op,coextrusion ");
...
//MAQUINAS
mysql_select_db($database_conexion1, $conexion1);
$query_maquinas = "SELECT * FROM maquina WHERE activo=0 AND proceso_maquina='2' ORDER BY id_maquina DESC";
...
<head>
  ...
  <script language="javascript" type="text/javascript">
    ...
    function tiemposD() {
      ...
      /*condicinal para restar o no, el desperdicio del peso inicial, esto depende si coextrusion en la OP se selecciono si o no */
      var f_restakilosD = "restakilosD()"
      if ('<?php echo $row_orden_produccion['coextrusion'] ?>' == 'NO') {
        f_restakilosD = ""
      }
      /* fin condicional */
      ...
      file.setAttribute("style", "width:65px");
      file.setAttribute("onChange", f_restakilosD); //var f_restakilosD que permite restar kilos o no, del peso inicial
      file.setAttribute("required", "required");
      f.appendChild(file);
    ...
    <table align="center" class="table table-bordered table-sm">
      ...
      <tr>
        <td colspan="4" id="numero2">ROLLO N&deg;
          <?php if ($row_orden_produccion['coextrusion'] != 'NO') : ?>
            <input name="rollo_r" type="hidden" id="rollo_r" step="any" value="<?php echo $row_Rollo_E['rollo_r'] ?>" required style="width:80px" />
          <?php endif; ?>
          <?php if ($row_orden_produccion['coextrusion'] == 'NO') : ?>
            <input name="idrollo" type="number" id="idrollo" min="1" value="<?php echo $row_rollo['rollo_r'] + 1 ?>" onChange="getClientIdNoexT(this.name,this.value); validar2();" onblur="valoridRollo();" style="width:60px" />
            <input name="rollo_r" type="hidden" id="rollo_r" min="1" value="<?php echo $row_rollo['rollo_r'] + 1 ?>" style="width:60px" /><!--  -->
          <?php else : ?>
          ...
          <!-- <input type="text" list="misdatos" name="rollo_r" id="rollo_r" value="<?php //echo $row_rollo['rollo_r']+1; 
                                                                                      ?>" onChange="getClientData(this.name,this.value)">
      ...
      <tr>
        ...
        <td colspan="4" id="fuente1"><?php $id_c = $row_op_carga['int_cliente_op'];
                                      ...
                                      $noextruder = $row_orden_produccion['coextrusion'];
      ...
      <tr>
        ...
        <td id="fuente1"><input name="metro_r" type="number" id="metro_r" min="1" style="width:100px" value="" required="required" <?php echo $noextruder=='SI' ? 'readonly'  : ''; ?> onChange="restakilosT()" /></td>
        <td id="fuente1" width='60px'><strong>METROS FINAL:</strong></td>
        <td id="fuente1" width='105px'><input name="metro_r2" type="number" id="metro_r2" min="1" style="width:100px" value="" required <?php echo $noextruder=='SI' ? 'readonly' : ''; ?> onChange="restakilosT()" /></td>
        <td id="fuente1">TIEMPO TOTAL</td>
        <td id="fuente1">&nbsp;</td>
      </tr>
      <tr>
        <td id="fuente1"><strong>PESO INICIAL:</strong></td>
        <td id="fuente7"><input name="kilos_r" type="number" id="int_kilos_prod_rp" min="1.00" step="0.01" style="width:100px" value="" <?php echo $noextruder=='SI' ? 'readonly' : ''; ?> onChange="restakilosT();restakilosD()" /></td>
        <td id="fuente1"><strong>PESO FINAL:</strong></td>
        <td id="fuente7"><input name="int_total_kilos_rp" type="number" id="int_total_kilos_rp" min="1.00" step="0.01" style="width:100px" value="" required <?php echo $noextruder=='SI' ? 'readonly' : ''; ?> onChange="restakilosT()" /></td>
      ...
      <tr>
        <td colspan="2" id="fuente4">
          ...
          <input name="int_totalKilos_tinta_rp" type="hidden" id="int_totalKilos_tinta_rp" step="any" value="<?php echo $row_totalTintas['tintas']; ?>" />
        </td>
        <td colspan="2" id="fuente3">&nbsp; </td>
      </tr>
      <tr>
        <td colspan="5" id="titulo1">DEFECTOS/BANDERAS</td>
      </tr>
      ...
      <!-- <tr>
        ...
        <td colspan="2" id="fuente1"><input type="number" name="tante_r" min="0" max="9" style="width:40px" value="0" onChange="sumaBanderasI();">
          <input name="tante2_r" type="number" id="tante2_r" style="width:60px" min="0" value="0">Metros
        </td>
      </tr>
      <tr>
        ...
        <td id="fuente1"><input type="number" name="manch_r" min="0" max="9" style="width:40px" value="0" onChange="sumaBanderasI();">
          <input name="manch2_r" type="number" id="manch2_r" style="width:60px" min="0" value="0">Metros
        </td>
        ...
        <td colspan="2" id="fuente1"><input type="number" name="color_r" min="0" max="9" style="width:40px" value="0" onChange="sumaBanderasI();">
          <input name="color2_r" type="number" id="color2_r" style="width:60px" min="0" value="0">Metros
        </td>
      </tr>
      <tr>
        ...
        <td id="fuente1"><input type="number" name="empat_r" min="0" max="9" style="width:40px" value="0" onChange="sumaBanderasI();">
          <input name="empat2_r" type="number" id="empat2_r" style="width:60px" min="0" value="0">Metros
        </td>
      ...
      <tr>
        ...
        <td id="fuente1"><input type="number" name="rasqueta_r" min="0" max="9" style="width:40px" value="0" onChange="sumaBanderasI();">
          <input name="rasqueta2_r" type="number" id="rasqueta2_r" style="width:60px" min="0" value="0">Metros
        </td>
