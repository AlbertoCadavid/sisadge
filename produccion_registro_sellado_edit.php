...
WHERE a.tipo_empleado IN(7,9) AND b.fecha BETWEEN DATE_FORMAT('$FECHA_NOVEDAD_SELL', '%Y-%m-01') AND DATE_FORMAT('$FECHA_NOVEDAD_SELL', '%Y-%m-31')"; //IN(5,10)novedad impresion 
  ...
  for ($t = 0; $t < count($cinta); $t++) {
    if (!empty($cinta[$t])) {
      $updateINV = sprintf(
        "UPDATE TblInventarioListado SET Salida = Salida - $metros_r + %s WHERE Codigo = %s",
  ...
  if (isset($_POST['id_r']) && $nuevovalorRollo) {
    ...
    //actualiza el campo de rollo a null para que en la pantala de tiquetes vuelva a pedir que escojan el nuevo rollo
    /* if ($_POST['rolloParcial_r'] == 0) {
      $objRollo->actualizarRegistro("tbl_numeracion", "rollo = null", "WHERE int_op_n = $_POST[id_op_rp]");
    } */
  } //FIN  
  ...
  if (!empty($_POST['id_rpt']) && !empty($_POST['valor_tiem_rt'])) {
    ...
    for ($i = 0; $i < count($a); $i++) {
      if (!empty($a[$i]) && !empty($b[$i])) { //no salga error con campos vacios
        $insertSQLt = sprintf(
          "INSERT INTO Tbl_reg_tiempo (id_rpt_rt,valor_tiem_rt,op_rt,int_rollo_rt,id_proceso_rt,fecha_rt) VALUES (%s, %s, %s, %s, %s, %s)",
  ...
  if (!empty($_POST['id_rtp']) && !empty($_POST['valor_prep_rtp'])) {
    ...
    for ($x = 0; $x < count($h); $x++) {
      if (!empty($h[$x]) && !empty($l[$x])) { //no salga error con campos vacios
        $insertSQLp = sprintf(
          "INSERT INTO Tbl_reg_tiempo_preparacion  (id_rpt_rtp,valor_prep_rtp,op_rtp,int_rollo_rtp,id_proceso_rtp,fecha_rtp) VALUES (%s, %s, %s, %s, %s, %s)",
...
<head>
  ...
  <script language="javascript" type="text/javascript">
    ...
    function tiemposD() {
      ...
      /*file0.setAttribute("onChange", "kiloComparativoSell()"); */
    ...
    function parcial() {
      swal({
        ...
        function(isConfirm) {
          ...
          //}
          document.getElementById("rolloParcial_r").checked = txt;
...
<body><!-- -->
  ...
  <form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form1" onSubmit="return(parcial() || kiloComparativoSell());"><!--  -->
    <table class="table table-bordered table-sm">
      ...
      <tr>
        <td colspan="2" id="dato2"><input type="hidden" name="ancho" id="ancho" style="width:80px" value="<?php echo $row_referencia['ancho_ref']; ?>" />
          <input type="hidden" name="calibre" id="calibre" style="width:80px" value="<?php echo $row_referencia['calibre_ref']; ?>" />
        </td>
        <td id="dato2">&nbsp;</td>
      </tr>
      <tr id="tr1">
      ...
      <tr>
        <td id="fuente1">
          ...
          <p>
            ...
            <input type="hidden" name="kiloInicial" id="kiloInicial" style="width:80px" value="<?php echo $row_rollo_sellado_edit['int_kilos_prod_rp']; ?>" />

            <input name="int_kilos_prod_rp" type="text" id="int_kilos_prod_rp" min="1.00" step="0.01" style="width:60px" value="<?php echo $row_rollo_sellado_edit['int_kilos_prod_rp']; ?>" required="required" readonly="readonly" />
          </p>
        </td>
        <td id="fuente1">
      ...
      <input name="valor_prep_rt" id="valor_prep_rt" type="hidden" size="5" value="0"/>-->
        ...
        <td id="fuente1"><select name="str_maquina_rp" id="maquina" style="width:155px">
            <option value="" <?php if (!(strcmp("", $row_rollo_sellado_edit['str_maquina_rp']))) {
                                echo "selected=\"selected\"";
                              } ?>>Maquina</option>
            <?php
            do {
            ?>
              <option value="<?php echo $row_maquinas['id_maquina'] ?>" <?php if (!(strcmp($row_maquinas['id_maquina'], $row_rollo_sellado_edit['str_maquina_rp']))) {
                                                                          echo "selected=\"selected\"";
        ...
        <td id="fuente1">
          <select name="int_cod_empleado_rp" id="operario" style="width:120px">
            <option value="" <?php if (!(strcmp("", $row_rollo_sellado_edit['int_cod_empleado_rp']))) {
                                echo "selected=\"selected\"";
                              } ?>>Operario</option>
            <?php foreach ($row_codigo_empleado as $row_codigo_empleado) { ?>
              <option value="<?php echo $row_codigo_empleado['codigo_empleado'] ?>" <?php if (!(strcmp($row_codigo_empleado['codigo_empleado'], $row_rollo_sellado_edit['int_cod_empleado_rp']))) {
                                                                                      echo "selected=\"selected\"";
        ...
        <td id="fuente1">
          ...
          <select name="int_cod_liquida_rp" style="width:120px" id="auxiliar">
            <option value="" <?php if (!(strcmp("", $row_rollo_sellado_edit['int_cod_liquida_rp']))) {
                                echo "selected=\"selected\"";
                              } ?>>Revisor</option>
            <?php foreach ($row_revisor as $row_revisor) { ?>
              <option value="<?php echo $row_revisor['codigo_empleado'] ?>" <?php if (!(strcmp($row_revisor['codigo_empleado'], $row_rollo_sellado_edit['int_cod_liquida_rp']))) {
                                                                              echo "selected=\"selected\"";
                                                                            } ?>><?php echo $row_revisor['codigo_empleado'] . " - " . $row_revisor['nombre_empleado'] . " " . $row_revisor['apellido_empleado'] ?></option>
            <?php } ?>
          </select>
        </td>
    ...
    <td id="fuente1"> 
      <select name="int_cod_liquida_rp" style="width:120px" id="auxiliar" >
        <option value=""<?php if (!(strcmp("", $rew_operarios['cod_auxiliar_r']))) {
                          echo "selected=\"selected\"";
                        } ?>>Revisor</option>
        <?php
        do {
        ?>
        <option value="<?php echo $row_codigo_empleado['codigo_empleado'] ?>"<?php if (!(strcmp($row_codigo_empleado['codigo_empleado'], $rew_operarios['cod_auxiliar_r']))) {
                                                                              echo "selected=\"selected\"";
                                                                            } ?>><?php echo $row_codigo_empleado['codigo_empleado'] . " - " . $row_codigo_empleado['nombre_empleado'] . " " . $row_codigo_empleado['apellido_empleado'] ?></option>
        <?php
        } while ($row_codigo_empleado = mysql_fetch_assoc($codigo_empleado));
      ...
      <tr>
        ...
        <td colspan="2" id="fuente1"><input name="fecha_ini_rp" id="fecha_ini_rp" type="datetime-local" min="2000-01-02" value="<?php echo muestradatelocal($row_rollo_sellado_edit['fechaI_r']); ?>" size="15" required onclick="restakilosT();" onChange="alerta_sell();" />
          <div id="resultado_generador"></div>
        </td>
        <td id="fuente1">
          <input type="number" name="rollo_rp" id="rollo_rp" min="0" step="any" required="required" readonly="readonly" placeholder="Rollos" style="width:46px" value="<?php echo $row_rollo_sellado_edit['rollo_r']; ?>" /> de
      ...
      <tr id="tr1">
        ...
        <td colspan="2" id="fuente1"><input name="fecha_fin_rp" id="fecha_fin_rp" type="datetime-local" min="2000-01-02" size="15" required onblur="restakilosT()" value="<?php echo muestradatelocal($row_rollo_sellado_edit['fechaF_r']); //muestradatelocal
                                                                                                                                                                          ?>" /></td>
        <td id="fuente1">
          <p>Metro Restante</p>
          <p>
      ...
      <tr>
        ...
        <td colspan="3" id="fuente1">
          <input name="total_horas_rp" id="total_horas_rp" type="text" required="required" readonly="readonly" placeholder="total horas" value="<?php echo $row_rollo_sellado_edit['total_horas_rp']; ?>" size="15" onclick="restakilosT();" />
          Tiempo Optimo
          <input name="rodamiento_rp" id="tiempoOptimo_rp" type="text" size="5" onclick="restakilosT();" required="required" placeholder="rodamientos" value="<?php echo $row_rollo_sellado_edit['rodamiento_rp']; ?>" readonly="readonly" />
        </td>
      </tr>
      <tr id="tr1">
        <td id="fuente1">Numeracion Inicial</td>
        <td colspan="2" id="fuente1">
          <input type="hidden" name="numInicioControl" id="numInicioControl" value="<?php echo $row_rollo_sellado_edit['numIni_r']; ?>" />
          ...
          <input id="porcentaje" name="porcentaje" type="hidden" value="<?php echo $row_rollo_sellado_edit['porcentaje_op_rp']; ?>" min="0" max="100" step="1" style="width:40px" required="required" readonly="readonly" />
        </td>
        <td id="fuente1">Numeracion Final
          <input type="text" name="n_fin_rp" id="n_fin_rp" style="width:116px" required="required" onBlur="conMayusculas(this)" value="<?php echo $row_rollo_sellado_edit['numFin_r']; ?>" />
      ...
      <tr>
        ...
        <td colspan="2" id="fuente1">
          <input type="text" name="int_kilos_desp_rp" id="int_kilos_desp_rp" min="0" step="any" required="required" size="7" placeholder="Desp.oper" value="<?php echo $row_total_desperdicio['desperdicio']; ?>" readonly="readonly" />
          <?php if ($row_usuario['tipo_usuario'] <> '5') { ?>
            <input type="number" name="kiloSistema" id="kiloSistema" style="width:55px" required="required" min="0.00" step="any" placeholder="Sistema" value="<?php echo $row_rollo_sellado_edit['kiloFaltante_rp']; ?>" readonly="readonly" /><?php } ?>
        </td>
        <td id="fuente1"><input name="int_kilosxhora_rp" type="number" required="required" id="int_kilosxhora_rp" min="0.10" step="any" value="<?php echo $row_rollo_sellado_edit['int_kilosxhora_rp']; ?>" style="width:116px" onblur="restakilosT();" />
          Kilos*Hora</td>
      </tr>
      <tr>
        <td id="fuente1">Rollo Parcial</td>
        <td id="fuente1"><input type="checkbox" name="rolloParcial_r" id="rolloParcial_r" <?php if (!(strcmp($row_rollo_sellado_edit['rolloParcial_r'], 1))) {
                                                                                            echo "checked=\"checked\"";
                                                                                          } ?> value="0" class="largerCheckbox" />
        </td>
        <td id="fuente1">&nbsp; </td>
        <td id="dato4">&nbsp;</td>
      </tr>
      <tr>
        ...
        <td id="dato4">&nbsp;</td>
        <td id="dato4">&nbsp;</td>
        <td id="dato3"><!-- <input type="button" name="ENVIAR" id="ENVIAR" value="EDITAR" parcial();"/> -->
          <input type="button" class="botonGeneral" name="ENVIAR" value="EDITAR" onclick="parcial();validaTodoSell()" />
        </td>
      </tr>
      <tr>
        <td colspan="7">
    ...
    <td colspan="13" id="fuente2"><a href="javascript:verFoto('produccion_regist_sellado_kilos_prod.php?id_op=<?php echo $row_rollo_sellado_edit['id_op_r'] ?>&rollo=<?php echo $row_rollo_sellado_edit['rollo_rp'] ?>&fecha=<?php echo $row_rollo_sellado_edit['fecha_ini_rp'] ?>','820','470')">
        ...
        <?php for ($s = 0; $s <= $totalRows_standBy - 1; $s++) { ?>
          <tr>
            ...
            <td id="fuente1">
              ...
              if ($numstand >= '1') {
                echo $Nombrestandby = mysql_result($resultstand, 0, 'nombre_rtp');
              } ?></td>
            <td id="fuente1"><?php $varST = mysql_result($standBy, $s, standby);
                              echo $varST;
                              $totalST += $varST; ?></td>
            <td id="fuente1"><!--ELIMINAR ESTA FUNCION eliminar_rts-->
              <a href="javascript:eliminar_rts('id_rts',<?php $delrt = mysql_result($standBy, $k, id_rt);
                                                        echo $delrt; ?>,'id_rs',<?php echo $row_rollo_sellado_edit['id_r']; ?>,'produccion_registro_sellado_edit.php')"><img src="images/por.gif" style="cursor:hand;" alt="ELIMINAR " title="ELIMINAR" border="0" /></a>
            </td>
          </tr>
        <?php } ?>
        <tr>
          ...
          <td id="fuente1">
            <strong>
              ...
              = <?php echo redondear_decimal($totalST / 60); ?> Horas</strong>
          </td>
          <td id="fuente1">&nbsp;</td>
        </tr>
      <?php } ?>
        ...
        <?php for ($k = 0; $k <= $totalRows_tiempoMuerto - 1; $k++) { ?>
          <tr>
            ...
            <td id="fuente1">
              ...
              if ($numtm >= '1') {
                $nombre1 = mysql_result($resulttm, 0, 'nombre_rtp');
                echo $nombre1;
              } ?>
              <strong>
            ...
            <td id="fuente1"><?php $var1 = mysql_result($tiempoMuerto, $k, muertos);
                              echo $var1;
                              $totalTM += $var1; ?>
        ...
        <?php for ($x = 0; $x <= $totalRows_tiempoPreparacion - 1; $x++) { ?>
          <tr>
            ...
            <td id="fuente1"><?php $id2 = mysql_result($tiempoPreparacion, $x, id_rpt_rtp);
                              ...
                              if ($numrtp >= '1') {
                                $nombre2 = mysql_result($resultrtp, 0, 'nombre_rtp');
                                echo $nombre2;
                              } ?> </td>
            <td id="fuente1"><?php $var2 = mysql_result($tiempoPreparacion, $x, preparacion);
                              echo $var2;
                              $totalTP += $var2; ?></td>
            <td id="fuente1"><a href="javascript:eliminar_varias('id_rps',<?php $delrp = mysql_result($tiempoPreparacion, $x, id_rt);
                                                                          echo $delrp; ?>,'id_rs',<?php echo $row_rollo_sellado_edit['id_r']; ?>,'produccion_registro_sellado_edit.php')"><img src="images/por.gif" style="cursor:hand;" alt="ELIMINAR " title="ELIMINAR" border="0" /></a>
            </td>
          </tr>
        <?php } ?>
        <tr>
        ...
        <?php for ($i = 0; $i <= $totalRows_desperdicio - 1; $i++) { ?>
          <tr>
            ...
            <td id="fuente1"><?php $id3 = mysql_result($desperdicio, $i, id_rpd_rd);
                              ...
                              if ($numrtd >= '1') {
                                $nombre3 = mysql_result($resultrtd, 0, 'nombre_rtp');
                                echo $nombre3;
                              } ?>
              <strong>
                <!--<input name="id_rpd[]" type="hidden" id="id_rpd[]" value="<?php echo $id3; ?>" size="6"/>-->
              </strong>
            </td>
            <td id="fuente1"><?php $var3 = mysql_result($desperdicio, $i, desperdicio);
                              echo $var3;
                              $totalTD += $var3; ?></td>
            <td id="fuente1"><a href="javascript:eliminar_varias('id_rds',<?php $delrd = mysql_result($desperdicio, $i, id_rd);
                                                                          echo $delrd; ?>,'id_rs',<?php echo $row_rollo_sellado_edit['id_r']; ?>,'produccion_registro_sellado_edit.php')"><img src="images/por.gif" style="cursor:hand;" alt="ELIMINAR " title="ELIMINAR" border="0" /></a></td>
          </tr>
        <?php } ?>
        <tr>
        ...
        <?php for ($y = 0; $y <= $totalRows_producido - 1; $y++) { ?>
          <tr>
            ...
            <td id="fuente1"><?php $id4 = mysql_result($producido, $y, id_rpp_rp);
                              ...
                              if ($numri >= '1') {
                                $nombre4 = mysql_result($resultri, 0, 'descripcion_insumo');
                                echo $nombre4;
                              } ?></td>
            <td id="fuente1"><?php $var4 = mysql_result($producido, $y, producido);
                              echo $var4;
                              $totalMM += $var4; ?></td>
            <td id="fuente1"><a href="javascript:eliminar_rts('id_ips',<?php $delip = mysql_result($producido, $y, id_rkp);
                                                                        echo $delip; ?>,'id_rs',<?php echo $row_rollo_sellado_edit['id_r']; ?>,'produccion_registro_sellado_edit.php')"><img src="images/por.gif" style="cursor:hand;" alt="ELIMINAR " title="ELIMINAR" border="0" /></a></td>
          </tr>
        <?php } ?>
        <tr>
      ...
      <tr>
        <td colspan="4" id="fuente1"><!--INSUMOS Y REFERENCIA-->
          ...
          <input type="hidden" name="ref_inven" id="ref_inven" value="<?php echo $row_referencia['cod_ref'] . "-" . $row_referencia['version_ref']; ?>" />
        </td>
      </tr>
      <tr>
