  ...
  for (i=0; i<(args.length-2); i+=3) { test=args[i+2]; val=MM_findObj(args[i]);
    if (val) { nm=val.name; if ((val=val.value)!="") {
      if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@');
        if (p<1 || p==(val.length-1)) errors+='- '+nm+' es un email.\n';
      } else if (test!='R') { num = parseFloat(val);
        if (isNaN(val)) errors+='- '+nm+' es numerico.\n';
...
function elimina(campo,id,pagina) 
{
	if(campo=='menu')
	{
	...
	if(campo=='n_cn')
	{
		...
		if(elim)
		{
			window.location ="delete2.php?"+ campo+"="+ id;
		}
		else
		{
			window.history.back();
...
{
	elim=confirm("¿Quieres Eliminar?");
	if(elim) { window.location ="delete2.php?"+ campo+"="+ id; }
...
{ 
  elim=confirm("¿Quieres Eliminar?"); 
  if(elim){ window.location ="delete2.php?"+ foranea1+"="+ foranea2+"&"+ campo+"="+ id;
...
{ 
  ...
  if(elim) 
  {
	  window.location ="delete2.php?"+campo+"="+ id+"&archivo="+ archivo;
  }
  else 
  {
	 if(campo=='egparchivo')
